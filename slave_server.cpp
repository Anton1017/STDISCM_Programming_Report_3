...
int wmain() {
    ...
    while (true) {
        ClientSocket = accept(ListenSocket, (SOCKADDR *) &clientService, &clientServiceSize);
        if (ClientSocket == INVALID_SOCKET) {
            wprintf(L"accept function failed with error: %d\n", WSAGetLastError());
            closesocket(ListenSocket);
            WSACleanup();
            return   1;
        }

        // For demonstration, just print a message and close the connection
        wprintf(L"Accepted connection from %s:%d\n", inet_ntoa(clientService.sin_addr), ntohs(clientService.sin_port));
